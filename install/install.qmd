...
On a Windows 10 machine, we strongly recommend first installing Anaconda or miniconda and then using the conda installer to be CmdStanR or RStan,
since the conda packages include a C++ toolchain.

Alternatively, R Users can get the RTools distribution, or install it from within CmdStanR using the `get_toolchain` method.
